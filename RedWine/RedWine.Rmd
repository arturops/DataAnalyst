...
Interestingly enough the volatile acidity seems to constantly reduce compare to the **quality** grade a wine gets.
...
The total sulfur dioxide seems not to have a clear correlation to the quality of a wine. it actually seems to be similar in all wines and it follows the same pattern of wines in each category. In other words, we have more wines 5 and 6 and also this show a higher total sulfur dioxide.
...
1. Alcohol - Positive. Higher alcohol level, higher quality
2. Volatile acidity - Negative. Lower volatile acidity, higher quality
3. Sulphates - Positive. Higher sulphates, higher quality
4. Citric acid - Positive. Higher citric acid, higher quality. [Negative correlation with Volatile acidity]
...
We start plotting our variables with strongest relation to quality.

```{r echo=FALSE, Multivariate_Plots_Alcohol_VolatileAcidity_Quality}
# plot fo two strongest relationship variables to quality: alcohol and volatile acidity
...
It seems that the lower the volatile acidity and the lower the alcohol the quality is good. However, the higher the alcohol gets the volatile acidity can also increase and the quality remains good.
...
From this plot, we can see that it is more common to have good wines after 10 % volume alcohol and the higher the alcohol level more chances of a grade 8 wine. Also, it seems grade 8 wines mainly appear when sulphates are in the range of 0.6 and 0.9.
